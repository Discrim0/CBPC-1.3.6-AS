...
Collision::Collision(NiAVObject* node, std::vector<Sphere> spheres, float actorWeight)
{
...
NiPoint3 Collision::CheckPelvisCollision(std::vector<Sphere> thingCollisionSpheres)
{
	...
	if (CollisionObject != nullptr)
	{
		...
		{
			IsItCollidingTriangleToSphere(collisionDiff, thingCollisionSpheres, collisionTriangles, 99, false);
		}
		#endif
	}
...
NiPoint3 Collision::CheckCollision(bool &isItColliding, std::vector<Sphere> thingCollisionSpheres, float timeTick, long deltaT, float maxOffset, bool maybe)
{
	...
	if (CollisionObject != nullptr)
	{
		...
		if (collisionTriangles.size() > 0)
		{
			isColliding = IsItCollidingTriangleToSphere(collisionDiff, thingCollisionSpheres, collisionTriangles, maxOffset, maybe);
			if (isColliding)
			{
				isItColliding = true;
